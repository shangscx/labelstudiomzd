...
class RichTextPieceView extends Component {
  ...
  _onMouseUp = (ev) => {
    ...
    Utils.Selection.captureSelection(({ selectionText, range }) => {
      ...
      if (item.type === 'text' && this.props.store.settings.autoCreateSame) {
        const taskObj = this.props.store.task.dataObj;
        const wholeText = taskObj.text;

        let pos = wholeText.indexOf(selectionText);

        console.log('wholeText, pos', wholeText, pos, selectionText);
        while (pos > -1) {
          if (soff === pos) {
            pos = wholeText.indexOf(selectionText, pos + selectionText.length);
            continue;
          }
          console.log('new pos', pos);
          const newRange = Utils.Selection.findRangeNative(pos, pos + selectionText.length, root);
  
          fixCodePointsInRange(newRange);
          const newnormedRange = xpath.fromRange(newRange, root);
  
          if (!normedRange) return;
  
          newnormedRange._range = newRange;
          newnormedRange.text = selectionText;
          newnormedRange.isText = item.type === "text";
          newnormedRange.dynamic = this.props.store.autoAnnotation;
  
          console.log('addRegion ', normedRange);
          item.addRegion(newnormedRange);
          pos = wholeText.indexOf(selectionText, pos + selectionText.length);
        }
      }
    }, {
